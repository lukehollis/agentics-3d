...
public class NetworkingController : MonoBehaviour
{
    ...
    private async Task InitializeWebSocket()
    {
        ...
        try 
        {
            ...
            websocket.OnError += (e) =>
            {
                ...
                GameController.Instance.SetOfflineMode(true);
                ...
                if (!GameController.Instance.hasStarted)
                {
                    ...
                    GameController.Instance.HandleGameLoaded();
                }
    ...
    private void OnMessageReceived(byte[] bytes)
    {
        ...
        try
        {
            ...
            else if (messageType == "load_game_response")
            {
                ...
                var loadGameResponse = JsonUtility.FromJson<LoadGameResponseData>(message);
                if (loadGameResponse?.message != null)
                {
                    // Convert the SaveData object back to JSON string for the SaveSystem
                    string saveDataJson = JsonUtility.ToJson(loadGameResponse.message);
                    SaveSystem.Instance.Load(saveDataJson);
                    GameController.Instance.HandleGameLoaded();
                }
                else
                {
                    Debug.LogError("Invalid load_game_response format");
                }
            }
        }
        catch (Exception ex)
    ...
    {
        var saveMessage = new SaveGameMessage
        {
            type = "save_game",
            user_uuid = user_uuid,
            game_session_uuid = game_session_uuid,
        ...
        var jsonString = JsonUtility.ToJson(saveMessage);
        SendWebSocketMessage(jsonString);
    }

    public void RequestSaveGameData()
    {
        var loadMessage = new SaveGameMessage
        {
            type = "load_game",
            user_uuid = user_uuid,
            game_session_uuid = game_session_uuid,
        ...
        var jsonString = JsonUtility.ToJson(loadMessage);
        SendWebSocketMessage(jsonString);
    }
