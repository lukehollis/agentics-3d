...
namespace Agentics
{
    public class Sensor : MonoBehaviour
    {
        ...
        public void CollectObservations(VectorSensor sensor)
        {
            ...
            sensor.AddObservation((int)agent.pathStatus);
        ...
        public float[] GetObservationData()
        {
            ...
            int navigationObservations = 4;
            int gridObservationsPerCell = 6;
            int totalGridObservations = gridObservationsPerCell * gridSize * gridSize;
            int observationSize = navigationObservations + totalGridObservations;
            ...
            observations[index++] = (float)agent.pathStatus;
            ...
            for (int x = 0; x < gridSize; x++)
            {
                ...
                {
                    var state = observationGrid[x, y];
                    observations[index++] = state.isOccupied ? 1f : 0f;
                    observations[index++] = state.isWalkable ? 1f : 0f;
                    observations[index++] = state.hasInteractable ? 1f : 0f;
                    observations[index++] = state.interactableType;
                    observations[index++] = state.distanceToAgent / viewRadius;
                    observations[index++] = state.isOnPath ? 1f : 0f;
                }
            }
        ...
        public GridState[,] GetObservationGrid()
        {
            UpdateObservationGrid();
